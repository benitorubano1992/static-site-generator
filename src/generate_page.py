...
def generate_pages_recursive(dir_path_content:str, template_path:str, dest_dir_path:str):
    if os.path.isfile(dir_path_content):
        return generate_page(dir_path_content,template_path,os.path.join(dest_dir_path,PATH_FILE))
    ...
    for el in elements_content:
        ...
        if os.path.isdir(path_el):
            generate_pages_recursive(path_el,template_path,os.path.join(dest_dir_path,el))
            continue
        generate_page(path_el,template_path,os.path.join(dest_dir_path,PATH_FILE))
